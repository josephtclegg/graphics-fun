function uvEarthCanvas() {
  ...
  const vertexShaderSource = `
    ...
    void main () {
      ...
      float u = 0.5 - 0.5 * atan(norm_coord.z, norm_coord.x) / (3.141592653589792);
  ...
  const createUvearth = function(slices, stacks) {
    const Z = 0.5;
    const R = Z;
    let SLICES = slices;
    let STACKS = stacks;
    ...
    earth.generateVertices(slices, stacks);
    return earth;
  };

  const canvas = document.getElementById('uvearthcanvas');
  ...
  earthImage.src = "./nasamercator.jpg";
  ...
  function render() {
    ...
    let earth = createUvearth(slices, stacks);
    //earth.generateVertices(slices, stacks);
    ...
    //console.log(earth.getIndices());

    for (let i=0; i<earth.getStacks(); i++) {
      gl.drawElements(gl.TRIANGLE_STRIP, (earth.getSlices()+1)*2, gl.UNSIGNED_SHORT, 2*i*((earth.getSlices()+1)*2));
    }
