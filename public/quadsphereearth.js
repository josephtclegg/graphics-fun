function quadSphereEarthCanvas() {
  ...
  const createCubesphereearth = function() {
    const Z = 0.5;
    const R = Z;
    ...
    let uvs = [
      64.0, 64.0,   128.0, 0.0,   64.0, 0.0,     //TOP
      128.0, 64.0,  128.0, 0.0,   64.0, 64.0,  
      64.0, 128.0,  128.0, 64.0,  64.0, 64.0,    //FRONT
      128.0, 128.0, 128.0, 64.0,  64.0, 128.0, 
      192.0, 128.0, 255.0, 64.0,  192.0, 64.0,   //BACK
      255.0, 128.0, 255.0, 64.0,  192.0, 128.0,
      0.0, 128.0,   64.0, 64.0,   0.0, 64.0,     //LEFT
      64.0, 128.0,  64.0, 64.0,   0.0, 128.0,  
      128.0, 128.0, 192.0, 64.0,  128.0, 64.0,   //RIGHT
      192.0, 128.0, 192.0, 64.0,  128.0, 128.0,
      64.0, 192.0,  128.0, 128.0, 64.0, 128.0,   //BOTTOM
      128.0, 192.0, 128.0, 128.0, 64.0, 192.0, 
    ];

    uvs = uvs.flatMap((n) => n/256.0);

    const cube_sphereearth = {
      getVertices() {
        return vertices;
      },
      getUvs() {
      ...
      subdivide() {
        ...
        * radius to curve them like a sphereearth :)
        *
        * A      E       B
        * +------+-------+
        * |      |       |
    ...
    return cube_sphereearth;
  };

  const canvas = document.getElementById('quadsphereearthcanvas');
  ...
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, 1, 1, 0, gl.RGBA, gl.UNSQIGNED_BYTE,
                new Uint8Array([0, 0, 255, 255]));

  var earthImage = new Image();
  earthImage.src = "./quadearth.png";
  ...
  function render() {
    gl.useProgram(shaderProgram);
    const earth = createCubesphereearth();
